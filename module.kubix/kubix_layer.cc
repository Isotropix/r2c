...
#include "./kubix_layer.cma"

IX_BEGIN_DECLARE_MODULE_CALLBACKS(KubixModuleLayer, ModuleLayerR2cSceneCallbacks)
    static R2cRenderDelegate *get_render_delegate(OfObject& object);
IX_END_DECLARE_MODULE_CALLBACKS(KubixModuleLayer)
...
{
    return new KubixRenderDelegate(&object.get_application());
}

namespace KubixLayer
{
    ...
    {
        OfClass *new_class = IX_DECLARE_MODULE_CLASS(KubixModuleLayer)
