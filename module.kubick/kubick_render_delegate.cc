...
public:
    ...
    struct {
        KubickResourceIndex index; // the index of all current resources where we store deduplicated data
    } resources;
    ...
    struct ClarisseToDummyObjectsMapping {
        ...
        CoreVector<R2cItemId> inserted; // is filled by KubickRenderDelegate::insert_xxx when a object is inserted to the scene
        CoreVector<R2cItemId> removed; // is filled by KubickRenderDelegate::remove_xxx when a object is removed from the scene
    ...
    ClarisseToDummyObjectsMapping<KubickLightIndex> lights;
    ClarisseToDummyObjectsMapping<KubickInstancerIndex> instancers;
};

IMPLEMENT_CLASS(KubickRenderDelegate, R2cRenderDelegate);

const CoreVector<CoreString> KubickRenderDelegate::s_supported_cameras      = { "CameraAlembic", "CameraUsd", "CameraPerspective", "CameraPerspectiveAdvanced"};
const CoreVector<CoreString> KubickRenderDelegate::s_unsupported_cameras    = {};
const CoreVector<CoreString> KubickRenderDelegate::s_supported_lights       = { "KubickLight" };
const CoreVector<CoreString> KubickRenderDelegate::s_unsupported_lights     = {};
const CoreVector<CoreString> KubickRenderDelegate::s_supported_materials    = { "KubickMaterial" };
const CoreVector<CoreString> KubickRenderDelegate::s_unsupported_materials  = {};
const CoreVector<CoreString> KubickRenderDelegate::s_supported_geometries   = { "SceneObject" };
const CoreVector<CoreString> KubickRenderDelegate::s_unsupported_geometries = { "GeometryBundle" };

KubickRenderDelegate::KubickRenderDelegate(OfApp *app) : R2cRenderDelegate()
{
    m = new KubickRenderDelegateImpl;
    m->app = app;
}

KubickRenderDelegate::~KubickRenderDelegate()
{
    clear();
    delete m;
}
...
CoreString 
KubickRenderDelegate::get_class_name() const
{
    return "KubickRenderer";
}

void
KubickRenderDelegate::insert_light(R2cItemDescriptor item)
{
    m->lights.inserted.add(item.get_id());
}

void
KubickRenderDelegate::remove_light(R2cItemDescriptor item)
{
    KubickLightInfo *light = m->lights.index.is_key_exists(item.get_id());
...
void
KubickRenderDelegate::dirty_light(R2cItemDescriptor item, const int& dirtiness)
{
    KubickLightInfo *light = m->lights.index.is_key_exists(item.get_id());
    if (light != nullptr) { // make sure it is indeed in our index
        m->lights.dirty = true;
...
void
KubickRenderDelegate::insert_instancer(R2cItemDescriptor item)
{
    m->instancers.inserted.add(item.get_id());
}

void
KubickRenderDelegate::remove_instancer(R2cItemDescriptor item)
{
    KubickInstancerInfo *instancer = m->instancers.index.is_key_exists(item.get_id());
...
void
KubickRenderDelegate::dirty_instancer(R2cItemDescriptor item, const int& dirtiness)
{
    KubickInstancerInfo *instancer = m->instancers.index.is_key_exists(item.get_id());
    if (instancer != nullptr) { // make sure it is indeed in our index
...
void
KubickRenderDelegate::insert_geometry(R2cItemDescriptor item)
{
    m->geometries.inserted.add(item.get_id());
}

void
KubickRenderDelegate::remove_geometry(R2cItemDescriptor item)
{
    KubickGeometryInfo *geometry = m->geometries.index.is_key_exists(item.get_id());
...
void
KubickRenderDelegate::dirty_geometry(R2cItemDescriptor item, const int& dirtiness)
{
    KubickGeometryInfo *geometry = m->geometries.index.is_key_exists(item.get_id());
    if (geometry != nullptr) { // make sure it is indeed in our index
...
float
KubickRenderDelegate::get_render_progress() const
{
    return m->progress.get_float();
}

void
KubickRenderDelegate::sync()
{
...
void
KubickRenderDelegate::sync_camera(const unsigned int& width, const unsigned int& height)
{
    m->camera.init_ray_generator(*get_scene_delegate(), width, height);
}

void
sync_shading_groups(const R2cSceneDelegate& delegate, R2cItemId cgeometryid, KubickGeometryInfo& rgeometry)
{
    ...
    if (shading_group.get_material().is_null()) {
        ...
        rgeometry.material = static_cast<ModuleMaterialKubick *>(shading_group.get_material().get_item()->get_module());
    }
}

void
KubickRenderDelegate::_sync_geometry(R2cItemId cgeometryid, KubickGeometryInfo& rgeometry, const bool& is_new)
{
    if (rgeometry.dirtiness & R2cSceneDelegate::DIRTINESS_GEOMETRY) {
        if (!is_new) {
            ...
            KubickResourceInfo *stored_resource = m->resources.index.is_key_exists(rgeometry.resource);
            if (stored_resource == nullptr) { // the resource doesn't exists so let's create it
                ...
                KubickResourceInfo new_resource;
...
void
KubickRenderDelegate::sync_geometries()
{
    if (m->geometries.is_dirty()) {
        ...
        for (auto removed_item : m->geometries.removed) {
            KubickGeometryInfo *geometry = m->geometries.index.is_key_exists(removed_item);
            ...
            if (geometry != nullptr) {
                ...
                KubickResourceInfo *stored_resource = m->resources.index.is_key_exists(geometry->resource);
        ...
        KubickGeometryInfo geometry;
        CoreVector<KubickGeometryInfo> new_geometries(0, m->geometries.inserted.get_count());
...
void
sync_shading_groups(const R2cSceneDelegate& delegate, R2cItemId cinstancerid, KubickInstancerInfo& rinstancer)
{
    ...
    if (shading_group.get_material().is_null()) {
        ...
        rinstancer.material = static_cast<ModuleMaterialKubick *>(shading_group.get_material().get_item()->get_module());
    }
}

void
KubickRenderDelegate::_sync_instancer(R2cItemId cinstancerid, KubickInstancerInfo& rinstancer, const bool& is_new)
{
    ...
    if (rinstancer.dirtiness & R2cSceneDelegate::DIRTINESS_GEOMETRY) {
        if (!is_new) { // existing instancer
            ...
            KubickResourceInfo *stored_resource = m->resources.index.is_key_exists(rinstancer.resource);
            if (stored_resource == nullptr) { // the resource doesn't exists so let's create it
                // create corresponding geometry resource according to the Clarisse geometry
                KubickResourceInfo new_resource;
...
void
KubickRenderDelegate::sync_instancers()
{
    if (m->instancers.is_dirty()) {
        ...
        for (auto removed_item : m->instancers.removed) {
            KubickInstancerInfo *instancer = m->instancers.index.is_key_exists(removed_item);
            ...
            if (instancer != nullptr) {
                ...
                KubickResourceInfo *stored_resource = m->resources.index.is_key_exists(instancer->resource);
                if (stored_resource != nullptr) { // there's a resource bound to the current geometry
                    stored_resource->refcount--;
                    if (stored_resource->refcount == 0) { // no one is using that resource anymore so let's delete it
        ...
        // is why it is very important to first sync the index, remove and finally add.
        // let's see if we have to create new geometries
        KubickInstancerInfo instancer;
        CoreVector<KubickInstancerInfo> new_instancers(0, m->instancers.inserted.get_count());
...
void
sync_light(const R2cSceneDelegate& delegate, R2cItemId clightid, KubickLightInfo& rlight)
{
...
void
KubickRenderDelegate::sync_lights()
{
    if (m->lights.is_dirty()) {
        // remove lights first
        for (auto removed_item : m->lights.removed) {
            KubickLightInfo *light = m->lights.index.is_key_exists(removed_item);
        ...
        // creating new lights
        KubickLightInfo light;
        CoreVector<KubickLightInfo> new_lights(0, m->lights.inserted.get_count());
        for (auto inserted_item : m->lights.inserted) {
            // create corresponding light according to the Clarisse light
            KubickUtils::create_light(*get_scene_delegate(), inserted_item, light);
...
void
KubickRenderDelegate::get_supported_cameras(CoreVector<CoreString>& supported_cameras, CoreVector<CoreString>& unsupported_cameras) const
{
...
void
KubickRenderDelegate::get_supported_materials(CoreVector<CoreString>& supported_materials, CoreVector<CoreString>& unsupported_materials) const
{
...
void
KubickRenderDelegate::get_supported_lights(CoreVector<CoreString>& supported_lights, CoreVector<CoreString>& unsupported_lights) const
{
...
void
KubickRenderDelegate::get_supported_geometries(CoreVector<CoreString>& supported_geometries, CoreVector<CoreString>& unsupported_geometries) const
{
...
public :
    ...
    KubickRenderDelegate::RenderData data;
    const KubickRenderDelegate *dummy_render_delegate;
...
void
KubickRenderDelegate::render(R2cRenderBuffer *render_buffer, const float& sampling_quality)
{
    ...
    ModuleRendererKubick *settings = static_cast<ModuleRendererKubick *>(renderer.get_item()->get_module());
...
void raytrace_objects(const GMathRay& ray, const CoreHashTable<R2cItemId, OBJECT_INFO>& index, const KubickResourceIndex& resources_index, double &closest_hit_t, GMathVec3d &closest_hit_normal, MaterialData& closest_hit_material)
{
    ...
    for (const auto object: index)
    {
        ...
        const KubickResourceInfo *resource_info = resources_index.is_key_exists(object_info.resource);
...
void
KubickRenderDelegate::render_region(RenderData& render_data, const unsigned int& thread_id) const
{
