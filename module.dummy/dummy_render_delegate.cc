...
public:
    ...
    struct {
        DummyResourceIndex index; // the index of all current resources where we store deduplicated data
    } resources;
    ...
    struct ClarisseToDummyObjectsMapping {
        ...
        CoreVector<R2cItemId> inserted; // is filled by DummyRenderDelegate::insert_xxx when a object is inserted to the scene
        CoreVector<R2cItemId> removed; // is filled by DummyRenderDelegate::remove_xxx when a object is removed from the scene
    ...
    ClarisseToDummyObjectsMapping<DummyLightIndex> lights;
    ClarisseToDummyObjectsMapping<DummyInstancerIndex> instancers;
};

IMPLEMENT_CLASS(DummyRenderDelegate, R2cRenderDelegate);

const CoreVector<CoreString> DummyRenderDelegate::s_supported_cameras      = { "CameraAlembic", "CameraUsd", "CameraPerspective", "CameraPerspectiveAdvanced"};
const CoreVector<CoreString> DummyRenderDelegate::s_unsupported_cameras    = {};
const CoreVector<CoreString> DummyRenderDelegate::s_supported_lights       = { "LightDummy" };
const CoreVector<CoreString> DummyRenderDelegate::s_unsupported_lights     = {};
const CoreVector<CoreString> DummyRenderDelegate::s_supported_materials    = { "MaterialDummy" };
const CoreVector<CoreString> DummyRenderDelegate::s_unsupported_materials  = {};
const CoreVector<CoreString> DummyRenderDelegate::s_supported_geometries   = { "SceneObject" };
const CoreVector<CoreString> DummyRenderDelegate::s_unsupported_geometries = { "GeometryBundle" };

DummyRenderDelegate::DummyRenderDelegate(OfApp *app) : R2cRenderDelegate()
{
    m = new DummyDelegateImpl;
    m->app = app;
}

DummyRenderDelegate::~DummyRenderDelegate()
{
    clear();
    delete m;
}
...
CoreString 
DummyRenderDelegate::get_class_name() const
{
    return "RendererDummy";
}

void
DummyRenderDelegate::insert_light(R2cItemDescriptor item)
{
    m->lights.inserted.add(item.get_id());
}

void
DummyRenderDelegate::remove_light(R2cItemDescriptor item)
{
    DummyLightInfo *light = m->lights.index.is_key_exists(item.get_id());
...
void
DummyRenderDelegate::dirty_light(R2cItemDescriptor item, const int& dirtiness)
{
    DummyLightInfo *light = m->lights.index.is_key_exists(item.get_id());
    if (light != nullptr) { // make sure it is indeed in our index
        m->lights.dirty = true;
...
void
DummyRenderDelegate::insert_instancer(R2cItemDescriptor item)
{
    m->instancers.inserted.add(item.get_id());
}

void
DummyRenderDelegate::remove_instancer(R2cItemDescriptor item)
{
    DummyInstancerInfo *instancer = m->instancers.index.is_key_exists(item.get_id());
...
void
DummyRenderDelegate::dirty_instancer(R2cItemDescriptor item, const int& dirtiness)
{
    DummyInstancerInfo *instancer = m->instancers.index.is_key_exists(item.get_id());
    if (instancer != nullptr) { // make sure it is indeed in our index
...
void
DummyRenderDelegate::insert_geometry(R2cItemDescriptor item)
{
    m->geometries.inserted.add(item.get_id());
}

void
DummyRenderDelegate::remove_geometry(R2cItemDescriptor item)
{
    DummyGeometryInfo *geometry = m->geometries.index.is_key_exists(item.get_id());
...
void
DummyRenderDelegate::dirty_geometry(R2cItemDescriptor item, const int& dirtiness)
{
    DummyGeometryInfo *geometry = m->geometries.index.is_key_exists(item.get_id());
    if (geometry != nullptr) { // make sure it is indeed in our index
...
float
DummyRenderDelegate::get_render_progress() const
{
    return m->progress.get_float();
}

void
DummyRenderDelegate::sync()
{
...
void
DummyRenderDelegate::sync_camera(const unsigned int& width, const unsigned int& height)
{
    m->camera.init_ray_generator(*get_scene_delegate(), width, height);
}

void
sync_shading_groups(const R2cSceneDelegate& delegate, R2cItemId cgeometryid, DummyGeometryInfo& rgeometry)
{
...
void
DummyRenderDelegate::_sync_geometry(R2cItemId cgeometryid, DummyGeometryInfo& rgeometry, const bool& is_new)
{
    if (rgeometry.dirtiness & R2cSceneDelegate::DIRTINESS_GEOMETRY) {
        if (!is_new) {
            ...
            DummyResourceInfo *stored_resource = m->resources.index.is_key_exists(rgeometry.resource);
            if (stored_resource == nullptr) { // the resource doesn't exists so let's create it
                ...
                DummyResourceInfo new_resource;
...
void
DummyRenderDelegate::sync_geometries()
{
    if (m->geometries.is_dirty()) {
        ...
        for (auto removed_item : m->geometries.removed) {
            DummyGeometryInfo *geometry = m->geometries.index.is_key_exists(removed_item);
            ...
            if (geometry != nullptr) {
                ...
                DummyResourceInfo *stored_resource = m->resources.index.is_key_exists(geometry->resource);
        ...
        DummyGeometryInfo geometry;
        CoreVector<DummyGeometryInfo> new_geometries(0, m->geometries.inserted.get_count());
...
void
sync_shading_groups(const R2cSceneDelegate& delegate, R2cItemId cinstancerid, DummyInstancerInfo& rinstancer)
{
...
void
DummyRenderDelegate::_sync_instancer(R2cItemId cinstancerid, DummyInstancerInfo& rinstancer, const bool& is_new)
{
    ...
    if (rinstancer.dirtiness & R2cSceneDelegate::DIRTINESS_GEOMETRY) {
        if (!is_new) { // existing instancer
            ...
            DummyResourceInfo *stored_resource = m->resources.index.is_key_exists(rinstancer.resource);
            if (stored_resource == nullptr) { // the resource doesn't exists so let's create it
                // create corresponding geometry resource according to the Clarisse geometry
                DummyResourceInfo new_resource;
...
void
DummyRenderDelegate::sync_instancers()
{
    if (m->instancers.is_dirty()) {
        ...
        for (auto removed_item : m->instancers.removed) {
            DummyInstancerInfo *instancer = m->instancers.index.is_key_exists(removed_item);
            ...
            if (instancer != nullptr) {
                ...
                DummyResourceInfo *stored_resource = m->resources.index.is_key_exists(instancer->resource);
                if (stored_resource != nullptr) { // there's a resource bound to the current geometry
                    stored_resource->refcount--;
                    if (stored_resource->refcount == 0) { // no one is using that resource anymore so let's delete it
        ...
        // is why it is very important to first sync the index, remove and finally add.
        // let's see if we have to create new geometries
        DummyInstancerInfo instancer;
        CoreVector<DummyInstancerInfo> new_instancers(0, m->instancers.inserted.get_count());
...
void
sync_light(const R2cSceneDelegate& delegate, R2cItemId clightid, DummyLightInfo& rlight)
{
...
void
DummyRenderDelegate::sync_lights()
{
    if (m->lights.is_dirty()) {
        // remove lights first
        for (auto removed_item : m->lights.removed) {
            DummyLightInfo *light = m->lights.index.is_key_exists(removed_item);
        ...
        // creating new lights
        DummyLightInfo light;
        CoreVector<DummyLightInfo> new_lights(0, m->lights.inserted.get_count());
        for (auto inserted_item : m->lights.inserted) {
            // create corresponding light according to the Clarisse light
            DummyUtils::create_light(*get_scene_delegate(), inserted_item, light);
...
void
DummyRenderDelegate::get_supported_cameras(CoreVector<CoreString>& supported_cameras, CoreVector<CoreString>& unsupported_cameras) const
{
...
void
DummyRenderDelegate::get_supported_materials(CoreVector<CoreString>& supported_materials, CoreVector<CoreString>& unsupported_materials) const
{
...
void
DummyRenderDelegate::get_supported_lights(CoreVector<CoreString>& supported_lights, CoreVector<CoreString>& unsupported_lights) const
{
...
void
DummyRenderDelegate::get_supported_geometries(CoreVector<CoreString>& supported_geometries, CoreVector<CoreString>& unsupported_geometries) const
{
...
public :
    ...
    DummyRenderDelegate::RenderData data;
    const DummyRenderDelegate *dummy_render_delegate;
...
void
DummyRenderDelegate::render(R2cRenderBuffer *render_buffer, const float& sampling_quality)
{
...
void raytrace_objects(const GMathRay& ray, const CoreHashTable<R2cItemId, OBJECT_INFO>& index, const DummyResourceIndex& resources_index, double &closest_hit_t, GMathVec3d &closest_hit_normal, MaterialData& closest_hit_material)
{
    ...
    for (const auto object: index)
    {
        ...
        const DummyResourceInfo *resource_info = resources_index.is_key_exists(object_info.resource);
...
void
DummyRenderDelegate::render_region(RenderData& render_data, const unsigned int& thread_id) const
{
